...
const addOption = () => {
    ...
    if (inputField.value.trim() !== '') {
        ...
        // ドラッグイベントのリスナーを追加
        newOption.addEventListener('dragstart', dragStart);
        newOption.addEventListener('dragover', dragOver);
        newOption.addEventListener('drop', drop);
    }
};
...
const drop = (event) => {
    ...
    if (dropzone && draggingElement !== dropzone) {
        ...
        newDraggingElement.addEventListener('dragstart', dragStart);
        newDraggingElement.addEventListener('dragover', dragOver);
        newDraggingElement.addEventListener('drop', drop);
    }
